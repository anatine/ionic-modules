...
import { matchComponent, matchRouteData, V5Match } from './utils/matcherUtils';
...
export class ReactRouterViewStack extends ViewStacks {
  constructor() {
    ...
    console.log(`🐣 ReactRouterViewStack created`);
  }
  ...
  createViewItem(
    ...
    if (!match) {
      console.warn(
        `☢️ createViewItem: Match was not found for ${routeInfo.pathname}`,
        reactElement
      );
    }
  ...
  findViewItemByRouteInfo(routeInfo: RouteInfo, outletId?: string) {
    // TEMP logging
    console.log(
      `findViewItemByRouteInfo: total view count 🔢🔢 ${
        this.getAllViewItems().length
      }`
    );
